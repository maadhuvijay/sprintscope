/**
 * Complete script to upload sprints.csv to Supabase
...
/**
 * Parse date from various formats (M/D/YYYY, YYYY-MM-DD, etc.)
 */
function parseDate(dateStr: string | null | undefined): string | null {
  ...
  try {
    // Handle M/D/YYYY format (common in CSV)
    if (dateStr.includes('/')) {
    ...
    if (dateStr.includes('-')) {
      ...
      if (/^\d{4}-\d{2}-\d{2}/.test(dateStr)) {
        return dateStr.split('T')[0]; // Extract just the date part if timestamp
      }
    }
...
async function main() {
  try {
    console.log('🚀 Starting sprints database upload...\n');
    ...
    if (checkError) {
      ...
      const migration1Path = path.join(process.cwd(), 'supabase/migrations/007_create_sprints_table.sql');
      const migration2Path = path.join(process.cwd(), 'supabase/migrations/008_setup_sprints_rls_policies.sql');
      
      if (fs.existsSync(migration1Path)) {
        const migration1SQL = fs.readFileSync(migration1Path, 'utf-8');
        console.log('   1️⃣  Migration 1: Create sprints table');
      ...
      console.log('\n   Then run this script again: npm run upload:sprints\n');
      process.exit(1);
    }

    console.log('✅ Sprints table exists\n');

    // Step 2: Read CSV
    const CSV_PATH = path.join(process.cwd(), 'sprints.csv');
    ...
    console.log('🗑️  Clearing existing sprints data...');
    ...
    const transformedRecords = records.map(record => ({
      ...
      sprint_name: record.sprint_name || null,
    ...
    console.log(`\n📊 Upload complete! ${records.length} sprints are now in your database.`);
